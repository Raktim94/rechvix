...
interface EInvoiceCredentialsStatus {
  Configured: boolean;
  GSTIN: string;
  Username: string;
  BaseURL: string;
...
/** Settings screen for internal/modules/einvoice/v1/sandbox — the real,
 * network-calling NIC e-Invoice API adapter, as opposed to the mock
 * provider apps/worker falls back to with no configuration at all. Saved
 * credentials are encrypted at rest (einvoice_provider_credentials,
 * migrations/0024) and never round-tripped back to this form — Configured
 * plus the two non-secret fields (GSTIN, Username) is all the status
 * check ever returns, so "Replace credentials" always starts from a
 * blank form, never a pre-filled secret. */
function EInvoiceProviderSection({ legalEntityId }: { legalEntityId: string }) {
  const queryClient = useQueryClient();
  ...
  const statusQueryKey = ["einvoice-credentials-status", legalEntityId];
  const status = useQuery({
    queryKey: statusQueryKey,
    queryFn: () => api.get<EInvoiceCredentialsStatus>(`/legal-entities/${legalEntityId}/einvoice-credentials`),
  });
  ...
  const save = useMutation({
    mutationFn: () =>
      api.put<EInvoiceCredentialsStatus>(`/legal-entities/${legalEntityId}/einvoice-credentials`, {
        client_id: clientId,
  ...
  const remove = useMutation({
    mutationFn: () => api.delete(`/legal-entities/${legalEntityId}/einvoice-credentials`),
    onSuccess: () => {
      queryClient.setQueryData(statusQueryKey, { Configured: false, GSTIN: "", Username: "", BaseURL: "" });
    },
  });

  const canSave = clientId && clientSecret && gstin && username && password;

  return (
    <div className={layout.panel}>
      <h2>e-Invoice provider</h2>
      <p className={layout.subtitle} style={{ marginBottom: 12 }}>
        By default every e-Invoice IRN is generated by a mock provider — safe for testing, but it never actually
        reaches the government (acknowledgement numbers start with "MOCKACK"). Configure real NIC e-Invoice API
        credentials here to submit for real.
      </p>
      {status.isPending ? (
        <p className={layout.subtitle}>Checking…</p>
      ) : status.data?.Configured && !showForm ? (
        <div>
          <p>
            Configured — GSTIN <strong>{status.data.GSTIN}</strong>, username <strong>{status.data.Username}</strong>
            {status.data.BaseURL ? (
              <>
                {" "}
                , endpoint <strong>{status.data.BaseURL}</strong>
              </>
            ) : null}
            .
          </p>
          <div className={ui.formActions}>
            ...
            <button type="button" className={ui.btnSecondary} disabled={remove.isPending} onClick={() => remove.mutate()}>
              {remove.isPending ? "Removing…" : "Remove — go back to mock"}
            </button>
          </div>
        </div>
      ) : (
        <div className={ui.formGrid}>
          <div className={ui.field}>
            <label htmlFor="einvoice-client-id">Client ID</label>
            <input id="einvoice-client-id" className={ui.input} value={clientId} onChange={(e) => setClientId(e.target.value)} autoComplete="off" />
          </div>
          <div className={ui.field}>
            <label htmlFor="einvoice-client-secret">Client secret</label>
            <input
              id="einvoice-client-secret"
              type="password"
              className={ui.input}
              value={clientSecret}
          ...
          <div className={ui.field}>
            <label htmlFor="einvoice-gstin">GSTIN</label>
            <input id="einvoice-gstin" className={ui.input} value={gstin} onChange={(e) => setGstin(e.target.value)} autoComplete="off" />
          </div>
          <div className={ui.field}>
            <label htmlFor="einvoice-username">e-Invoice portal username</label>
            <input id="einvoice-username" className={ui.input} value={username} onChange={(e) => setUsername(e.target.value)} autoComplete="off" />
          </div>
          <div className={ui.field}>
            <label htmlFor="einvoice-password">e-Invoice portal password</label>
            <input
          ...
          <div className={ui.field}>
            <label htmlFor="einvoice-base-url">API endpoint (optional)</label>
            <input
              id="einvoice-base-url"
              className={ui.input}
              placeholder="Defaults to NIC's sandbox — https://einv-apisandbox.nic.in"
              value={baseUrl}
          ...
          <div className={ui.formActions}>
            ...
            <button type="button" className={ui.btnPrimary} disabled={!canSave || save.isPending} onClick={() => save.mutate()}>
      ...
      {save.isError ? (
        <p role="alert" style={{ color: "var(--color-negative)", marginTop: 8 }}>
          {save.error instanceof ApiError ? save.error.message : "Could not save these credentials."}
        </p>
      ) : null}
      {remove.isError ? (
        <p role="alert" style={{ color: "var(--color-negative)", marginTop: 8 }}>
          {remove.error instanceof ApiError ? remove.error.message : "Could not remove these credentials."}
        </p>
      ) : null}
    </div>
  );
...
export function GstPage() {
  ...
  return (
    <div className={layout.page}>
      ...
      <EWayBillModeSection />
      {legalEntityId ? <EInvoiceProviderSection legalEntityId={legalEntityId} /> : null}
