...
// UserRoleScope is one user_roles row to write via
// ReplaceUserCompanyAccess — ID is the row's own id (pre-generated by the
// caller, matching this codebase's convention of generating ids at the
// app layer, not inside a repo method), LegalEntityID nil means
// unrestricted (same nil-means-unrestricted convention
// internal/platform/permissions' Grant/Scope already use throughout).
...
type RoleRepository interface {
	...
	// ReplaceUserCompanyAccess atomically replaces every user_roles row
	// this user holds for roleID with exactly the rows given — used both
	// when first creating a team member and when editing an existing
	// one's company access later, so "add" and "edit" never drift into
	// two different code paths that could disagree about what "no rows
	// left" means.
	ReplaceUserCompanyAccess(ctx context.Context, organisationID, userID, roleID uuid.UUID, rows []UserRoleScope, at time.Time) error
	// ListUserCompanyAccess is ReplaceUserCompanyAccess's read side —
	// unrestricted=true (any row with a NULL legal_entity_id) means every
	// company in the organisation, matching
	// permissions.Checker.AllowedLegalEntities' own contract exactly
	// (same nil-means-everything rule), just keyed by a specific target
	// user for the Settings UI instead of "the calling principal."
	ListUserCompanyAccess(ctx context.Context, organisationID, userID, roleID uuid.UUID) (unrestricted bool, legalEntityIDs []uuid.UUID, err error)
}
